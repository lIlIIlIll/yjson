...
YJson.toJson / fromJson<T> / *With / stream APIs
        ...
        generated, built-in or custom JsonCodec<T>
                         │
                         ▼
          one semantic reader / writer state machine
             ┌───────────┴───────────┐
             ▼                       ▼
      string / bytes / stream     optional Native primitives
```
...
Native primitive 可驱动 managed Compact builder；临时 Native 资源在返回前释放。
`materialize()` 会把 read-only document 转成完整 `JsonNode`。

## Native scanner seam

core 没有 C foreign declaration。`YJsonNativeAccel.initialize()` 在首次普通调用前安装
版本化 provider，并把进程状态从 `Unconfigured` 冻结为 `NativeFrozen`；否则首次普通调用
冻结为 `PureFrozen`。相同 provider 初始化可幂等重复，不支持卸载或运行期切换。底层
contract 见 [Native internals](maintainers/native-internals.md)。
