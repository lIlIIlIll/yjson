...
发布验证包含畸形输入测试、无编译警告的 Clang/GCC、ASan、UBSan、LSan、固定种子的
差分模糊测试、分配失败和生命周期、符号隔离。若发布说明包含 Native 加速声明，
还必须执行固定 CPU 的 Pure/Native 交替性能检查。发布证据记录对应的源码、SDK、
运行器和校验和；性能结论仅适用于该次验证的版本与环境。普通 Release 未声明 Native
加速时，Pure/Native 结果只作记录，不额外形成 qualification 门禁，具体规则见发布流程。
