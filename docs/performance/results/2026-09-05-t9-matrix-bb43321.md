...
主表 `jackson` 列为手写计时轨(`run_t9_jackson.py`);JMH 口径(1 fork、
2×1s warmup、5×1s measurement)单独成轨,仅覆盖 30/34 case,见文末对照。
同一 case 两种口径的 Jackson 数字差异见下文 JMH 段:hand 轨整体偏悲观
约 10%,因此**主表对 yjson 的相对位置偏有利**;JMH 口径下
yjson msgc / Jackson geomean ≈ 1.13(慢约 13%)。
...
## Jackson JMH 对照(主表口径校准)

JMH(1 fork、2×1s warmup、5×1s measurement,AverageTime µs/op)与手写计时轨
覆盖同 30 个 case。JMH/hand geomean = **0.899**:hand 轨把 Jackson 测慢了约
10%(hand 数字偏大),即 JMH 口径下 Jackson 更快。按 30 个共同 case 重算,
yjson msgc geomean 1.415µs vs Jackson JMH geomean 1.249µs,比率 ≈ **1.13**;
主表 hand 口径的 1.018 应据此理解。逐 case 表见归档 `server/jackson-jmh/jmh-deviation.md`。
