...
def has_bound_parameter_documentation(
    declaration: dict, parameter: dict
) -> bool:
    """Accept cjdoc's bound @param tag representation.

    cjdoc Doc IR v8 keeps the description on the declaration's documentation
    tag (`name=param`, `subject=<parameter>`); the parameter object's
    `documentation` field is null for this provider.
    """
    if nonempty(parameter.get("documentation")):
        return True
    documentation = declaration.get("documentation")
    if not isinstance(documentation, dict):
        return False
    parameter_name = parameter.get("name")
    return any(
        isinstance(tag, dict)
        and tag.get("name") == "param"
        and tag.get("subject") == parameter_name
        and nonempty(tag.get("description"))
        for tag in documentation.get("tags", [])
    )
...
def validate(document: dict, required: dict[str, dict[str, int]]) -> dict:
    ...
    for declaration in selected:
        ...
        for parameter in declaration.get("parameters", []):
            parameter_count += 1
            if not has_bound_parameter_documentation(declaration, parameter):
