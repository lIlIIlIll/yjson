#!/usr/bin/env python3
"""Summarize yjson DOM backend raw reports with yyjson as the reference."""
...
CASE_INFO = {
    ...
    for operation, entries in {
        "parse": (("pureAstParseLifecycle", "pure_ast"), ("pureCompactParseLifecycle", "pure_compact"),
                  ("customNativeParseLifecycle", "custom_native"), ("yyjsonDirectParseLifecycle", "yyjson_direct")),
        "lookup": (("pureAstRootLookup", "pure_ast"), ("pureCompactRootLookup", "pure_compact"),
                   ("customNativeRootLookup", "custom_native"), ("yyjsonDirectRootLookup", "yyjson_direct")),
        "traversal": (("pureCompactTraversal", "pure_compact"),
                      ("customNativeTraversal", "custom_native"), ("yyjsonDirectTraversal", "yyjson_direct")),
        "serialize": (("pureAstSerialize", "pure_ast"), ("pureCompactSerialize", "pure_compact"),
                      ("customNativeSerialize", "custom_native"), ("yyjsonDirectSerialize", "yyjson_direct")),
        "roundtrip": (("pureAstRoundTrip", "pure_ast"), ("pureCompactRoundTrip", "pure_compact"),
...
def analyze(root: Path, min_runs: int) -> list[dict[str, object]]:
    ...
    for case, (operation, backend) in CASE_INFO.items():
        ...
        reference_case = next(
            candidate for candidate, info in CASE_INFO.items()
            if info == (operation, "yyjson_direct")
        )
        if reference_case not in case_runs:
            raise ValueError(f"operation {operation!r} has no yyjson reference case")
        reference_runs = {
        ...
        rows.append({
            ...
            "latency_ratio_to_yyjson": statistics.median(ratios),
            "slower_than_yyjson_pairs": sum(ratio > 1.0 for ratio in ratios),
            "pairs": len(ratios),
...
def render_markdown(rows: list[dict[str, object]]) -> str:
    lines = [
        "`latency ratio` is backend median / yyjson Direct median; below 1 is faster than yyjson.",
        "",
    ...
    for row in rows:
        lines.append(
            ...
            f"{float(row['latency_ratio_to_yyjson']):.3f}x | "
            f"{row['slower_than_yyjson_pairs']}/{row['pairs']} |"
        )
    return "\n".join(lines) + "\n"
